...
BOOTSTRAP_CI_LIB_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=common.sh
# shellcheck disable=SC1091
source "${BOOTSTRAP_CI_LIB_DIR}/common.sh"
...
bootstrap_ci_prepare_directories() {
  local data_dir=$1
  local backups_dir=$2
  mkdir -p "${data_dir}/postgres_data" "${data_dir}/postgres_wal" "${data_dir}/pgbackrest" "${data_dir}/rabbitmq_data" "${data_dir}/valkey_data" "${data_dir}/prometheus" "${data_dir}/grafana"
