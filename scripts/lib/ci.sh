...
source "${CI_LIB_DIR}/common.sh"
# shellcheck source=ci_metadata.sh
# shellcheck disable=SC1091
source "${CI_LIB_DIR}/ci_metadata.sh"
# shellcheck source=ci_ports.sh
# shellcheck disable=SC1091
source "${CI_LIB_DIR}/ci_ports.sh"
...
USAGE
  ...
  if ! ci_check_required_ports "${skip_ports}"; then
    return 1
  fi
...
USAGE
  ...
  for service in postgres_exporter pgbouncer_exporter valkey_exporter memcached_exporter node_exporter cadvisor prometheus grafana; do
    if ! wait_for_service_healthy "${service}" "${MONITORING_HEALTH_TIMEOUT:-180}" 2; then
      echo "[ci] required monitoring service ${service} failed to pass health check." >&2
      return 1
    fi
  done
