...
- Postgres image tag is `${POSTGRES_IMAGE_NAME:-core_data/postgres}:${POSTGRES_IMAGE_TAG:-18.4-bookworm-core}`. CI builds/publishes to GHCR; attestations can be checked with:
  ...
  ```
- Companion services are also published under the same tag, e.g.:
  - `ghcr.io/paudley/core_data/valkey:<tag>`
...
- When GitHub reports “token was denied access…” (common on runner-scoped `GITHUB_TOKEN`/`GH_TOKEN`), the helper logs the failure and retries anonymously so public attestations continue without extra configuration.
