...
- **Required monitoring.** Prometheus scrapes PostgreSQL, PgBouncer, logical backups, RabbitMQ, ValKey, Memcached, host metrics, and container metrics. Grafana is provisioned with the Prometheus datasource and a Core Data overview dashboard. Prometheus keeps 7 days of samples by default; configure ports, retention, and credentials with the `PROMETHEUS_*`, `GRAFANA_*`, and `*_EXPORTER_*` variables.
...
| `backup [--verify]` / `stanza-create` / `restore-snapshot` | Manage pgBackRest backups & optionally restore the latest backup into a throwaway data dir for checksum verification. Defaults retain 7 days of full backups with matching archive cleanup. |
